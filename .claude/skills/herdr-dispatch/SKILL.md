...
- **`--permission-mode acceptEdits`**, not `--dangerously-skip-permissions`, for the same classifier reason. This means a dispatched sub-agent may still pause waiting for approval on non-edit actions — confirmed live: **every single `herdr agent send` / `herdr pane send-keys` a sub-agent runs (including the reporting-back protocol itself) triggers a Bash approval prompt** under `acceptEdits`, since those aren't file edits. A fully unattended chain therefore needs a human (or the Orchestrator, standing in) to click through each one, or the user pre-approving a permission rule for `herdr agent *` / `herdr pane *` in that sub-agent's own session (offered inline in the prompt: "Yes, and don't ask again for: ..."). Watch for `agent_status: "blocked"` via `herdr agent wait`, not just `"idle"`.
